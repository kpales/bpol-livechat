...
function addLastMsg(msgID,user,nowTime){
    chats[activeChat].lastMsg = {id: msgID, msgid:'msg_' + msgID, conid:'con_' + msgID, user: user, time:nowTime};
}

function addMsg(txt, msgUserId, msgStaffId, sendTo, msgTime, userChatID, specialchars, activeChatfalse){

    var out,appTO,msgID,chatData,activeUser,activeUserId,lastMsgUser,dataUsers, activeChatX, lastActiveChat;
    nowTime = new Date();
    lastActiveChat = activeChat;
    activeChat = userChatID;
    if (typeof(sendTo)==='undefined') sendTo = true;
    ...
    if (typeof(chats[activeChat].lastMsg.user) === 'undefined'){
        newIndex = 0;
    }else{
        if(chats[activeChat].lastMsg.user !== activeUser)
            newIndex = 0;
        else
            newIndex = 1;
    }
    ...
    if(newIndex === 0) {
        newIndex = 1;
        msgID = chats[activeChat].chatID + '_' + inc();
        chatData = {json:'1', msg:txt, chatID:chats[activeChat].chatID};
        ...
        chatData = {json:'1', msg:txt, chatID:chats[activeChat].chatID, subMsg: true};
        var diff = Date.parse(msgTime) - Date.parse(chats[activeChat].lastMsg.time);
        if(diff > 60000){
            msgID = chats[activeChat].chatID + '_' + inc();
            appTO = document.getElementById(chats[activeChat].lastMsg.conid);
            addLastMsg(msgID,activeUser,msgTime);
            out = generateMsg({id: msgID, nowTime: msgTime, msg: txt, userId: activeUserId, user: activeUser},true, specialchars);
        }else {
            chats[activeChat].lastMsg.time = msgTime;
            if(specialchars)
                out = '<div>' + emoticons(htmlspecialchars(txt).linkify(),emoticonsData) + '</div>';
            else
                out = '<div>' + emoticons(txt.linkify(),emoticonsData) + '</div>';
            appTO = document.getElementById(chats[activeChat].lastMsg.msgid);
        }
    }
    ...
    if(activeTabId != chats[activeChat].chatID)
        flashIt(chats[activeChat].chatID, 1);
