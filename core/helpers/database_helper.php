...
function installMySQLdb($con, $filePath, $gzip=false, $dbPrefix=false,$dbPrefixData=''){
    ...
    foreach ($lines as $line){
        ...
        if($dbPrefix)
            $templine = str_replace('pinky_',$dbPrefixData,$templine);
...
function deleteRowID($con, $tableName, $id){
    $tableName = DB_PREFIX.$tableName;
    $sql = 'DELETE FROM ' . $tableName . ' WHERE id='.$id;
    return mysqli_query($con, $sql);
}

function deleteRow($con, $tableName, $cond){
    $tableName = DB_PREFIX.$tableName;
