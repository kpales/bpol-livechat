...
function dbConnect($mysql_host=DB_HOST,$mysql_user=DB_USER,$mysql_pass=DB_PASS,$mysql_database=DB_NAME){
...
function getDaysOnThisMonth($month = 5, $year = '2018'){
...
function getLastID($con,$table) {
    ...
    $result = mysqli_query($con, "SELECT @last_id := MAX(id) FROM $table");
    $row = mysqli_fetch_array($result);
    return $row['@last_id := MAX(id)'];
}
